...
SCENARIOS = [
  ...
  dict(slug="directions", theme="travel", level=("A1", "B1"), minutes=5, sort=2,
  ...
  dict(slug="restaurant", theme="food", level=("A1", "B1"), minutes=7, sort=3,
  ...
  dict(slug="hotel", theme="travel", level=("A2", "B2"), minutes=6, sort=4,
...
sql = ("-- Generated by scripts/gen-seed-scenarios.py · the four design scenarios in fr / en / es.\n"
       "insert into public.scenarios (slug, language, title, theme, level_min, level_max, minutes, is_placement, illustration_storage_path, subtitle, brief, tasks, pip_prompt, sort_order) values\n"
       + ",\n".join(rows) + "\n"
       ...
       "  minutes = excluded.minutes, is_placement = excluded.is_placement, illustration_storage_path = excluded.illustration_storage_path, subtitle = excluded.subtitle,\n"
