...
# kind, theme, level window, minutes, sort, subtitle, brief, tasks (text i18n + hint per learning language), title + pip_prompt per learning language
SCENARIOS = [
  dict(kind="introduce", theme="social", level=("A1", "A2"), minutes=6, sort=1,
  ...
  dict(kind="directions", theme="travel", level=("A1", "B1"), minutes=5, sort=2,
  ...
  dict(kind="restaurant", theme="food", level=("A1", "B1"), minutes=7, sort=3,
  ...
  dict(kind="hotel", theme="travel", level=("A2", "B2"), minutes=6, sort=4,
...
for s in SCENARIOS:
    for lang in ("fr", "en", "es"):
        ...
        rows.append(f"  ({q(s['kind'])}, {q(lang)}, {q(title)}, {q(s['theme'])}, {q(s['level'][0])}, {q(s['level'][1])}, {s['minutes']}, "
                    f"{q('scenarios/' + s['kind'] + '.webp')}, {j(s['subtitle'])}, {j(s['brief'])}, {j(tasks)}, {q(prompt)}, {s['sort']})")

sql = ("-- Generated by scripts/gen-seed-scenarios.py · the four design scenarios in fr / en / es.\n"
       "insert into public.scenarios (kind, language, title, theme, level_min, level_max, minutes, illustration_storage_path, subtitle, brief, tasks, pip_prompt, sort_order) values\n"
       + ",\n".join(rows) + "\n"
       "on conflict (kind, language) do update set\n"
