...
for s in SCENARIOS:
    for lang in ("fr", "en", "es"):
        ...
        rows.append(f"  ({q(s['slug'])}, {q(lang)}, {q(title)}, {q(s['theme'])}, {q(s['level'][0])}, {q(s['level'][1])}, {s['minutes']}, "
...
sql = ("-- Generated by scripts/gen-seed-scenarios.py · the four design scenarios in fr / en / es.\n"
       "insert into public.scenarios (slug, language, title, theme, level_min, level_max, minutes, illustration_storage_path, subtitle, brief, tasks, pip_prompt, sort_order) values\n"
       + ",\n".join(rows) + "\n"
