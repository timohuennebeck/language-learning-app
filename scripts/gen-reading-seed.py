...
cards = []
for front, lemma, example, box, due_in, reviews, lapses in DECK:
    pos, _, trans, _, _ = VOCAB[lemma]
    due = 'current_date' if due_in == 0 else f'current_date + {due_in}'
    cards.append(
        f"  ({sql_str(front)}, '{lex_id(lemma, pos)}', {sql_str(trans)}, {sql_str(example)}, "
...
insert into public.flashcards
  (user_id, language, front, back, back_language, example, lexeme_id, box, due, reviews, lapses,
   last_reviewed_at)
select {DEV_USER}, 'fr', v.front, v.back, 'de', v.example, v.lexeme_id::uuid, v.box, v.due,
       v.reviews, v.lapses,
